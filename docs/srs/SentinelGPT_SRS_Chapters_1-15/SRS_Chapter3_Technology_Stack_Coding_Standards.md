...
- **Fallback explanations are pre-written, reviewed, deterministic templates** stored in code (not themselves AI-generated at request time), so a Gemini outage or validation failure degrades to a known-safe, human-reviewed explanation rather than an unpredictable one. ("Reviewed" means template-authorship review; it is distinct from live-transcript human review in research evaluation.)
